...
이 프로젝트의 규칙:
- **`.mcp.json`** — 공유용. **비밀값 없음**(현재 빈 `{}` 또는 비밀 없는 서버만).
- **`.pi/mcp.json`** — OAuth 클라이언트 ID/비밀 **리터럴**로 박는다. **`.gitignore`에 등록**(절대 커밋 금지).
...
[ ] .env 에 GMAIL_OAUTH_CLIENT_ID / SECRET
[ ] .pi/mcp.json 에 gmail(리터럴) + gmail-send 등록   ← .gitignore 확인

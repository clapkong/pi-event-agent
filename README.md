...
- **실제 동작**: 입력 → WS → Pi 텍스트 스트리밍(M1) · 모델 레지스트리·폴백 · `estimate_budget`(custom tool)
- **UI만 구현 (mock — 실제 기능 아님)**: 홈 · 기획 폼 · 동작 타임라인(되묻기·승인·모델 전환) · 작업공간·상태 모델 · 결과 문서
- **미구현**: 실제 Extension 모듈 · Skill · MCP · pgvector RAG · 승인 발송 · 사례 순환 (B3·B4)

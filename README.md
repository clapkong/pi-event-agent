...
> **선택 시나리오** — 과제에 제시된 20가지 시나리오에는 없는 **'행사 기획 도우미'를 별도 시나리오로 선정**해, 교수님께 사전 승인을 받았습니다.
...
- **과거 사례 재사용** — 끝난 행사는 사례 파일로 저장해 두었다가, 새 기획에서 검색해 인용합니다. 인용 시 사례에 어떤 행사가 썼는지(`citedBy`)가 기록됩니다.
...
9. **운영 중 점검 (행사 준비 기간)** — 작업 공간의 '점검'을 누르거나(또는 일정 주기마다 자동으로), **`secretary`가 받은 편지함에서 이 행사 관련 메일을 정리**(통신 카드)하고 **`monitor`가 날씨·통신 변화를 보고 다시 기획이 필요한지 판단**합니다. 영향이 있으면 "다시 검토 필요" 배너가 떠, 잠근 값은 그대로 둔 채 남은 예산만 다시 짭니다.
10. **행사 후 적립 (사례 순환 닫기)** — 사용자가 "완료"로 확정하면(완료 판단은 사람 몫), 에이전트가 간단한 회고·만족도를 받고(`satisfaction-survey` 스킬), 이 행사를 **조건·핵심 결정·예산 실집행·교훈**이 담긴 사례 파일(`cases/<id>.md`)로 정리해 **`save_case`로 저장 → `rag_index`로 검색 색인에 추가**합니다. → 다음 행사 기획의 1번 단계에서 이 사례가 다시 검색·인용됩니다(순환).
...
| `secretary` | 행사 운영 중, 받은 메일을 읽어 **회의록(사실 요약)**으로 정리 |
| `monitor` | 행사 운영 중, 날씨·통신 변화를 감지해 **다시 기획이 필요한지 판단** |

> **기획 단계**에선 메인이 `researcher`·`writer`·`critic`을 직접 호출하고, `writer ⇄ critic`은 "초안 작성 → 검토 → 수정"을 통과할 때까지(최대 2회) 반복합니다.
> **운영 단계**(행사 준비 기간)에선 작업 공간의 **'점검'**(수동 버튼 + 주기 자동)이 `secretary`(메일 정리)와 `monitor`(변화 감지·재기획 판단)를 호출해, 통신·날씨 변화를 보드에 반영합니다.

### 3.2 Skill — 에이전트의 업무 매뉴얼

Skill은 에이전트가 특정 작업을 일관되게 수행하도록 돕는 **절차 + 바로 쓰는 리소스(템플릿·데이터·레퍼런스) 묶음**입니다(`.pi/skills/<이름>/SKILL.md`). 직접 4개를 작성했고, 각 스킬은 절차 문서만이 아니라 `assets/`·`references/`에 실제 자료를 함께 담습니다.

| Skill | 절차 | 함께 담긴 리소스 |
|---|---|---|
| `budget-policy` | 행사 유형별 예산 배분 비율·조정 원칙 | `assets/ratios.json` — **`estimate_budget` 도구가 직접 읽는 배분 데이터**. 기업 행사 + **학생회 행사**(새터·MT·축제/주점·체전) 유형별 표 |
...
> DETAILS가 정의하는 Skill("지식·절차·스크립트·리소스 묶음")에 맞춰, 단순 지시가 아니라 **도구가 읽는 데이터(`ratios.json`)·메일 템플릿·점검 레퍼런스**까지 포함합니다. 에이전트는 작업을 만나면 해당 `SKILL.md`를 읽고 필요한 리소스를 펼쳐 씁니다(필요할 때만 불러오는 방식).
...
**안전장치(가드)는 Pi의 훅(Hook)으로 구현**했습니다 — `pi.on("tool_call", …)`으로 도구 실행을 가로채, 규칙을 어기면 `{ block: true }`로 **실행 자체를 막습니다.** 세 가지 훅이 작동합니다:

- **잠금 가드** — 사람이 확정🔒한 값(예산 항목·조건)이나 계약 완료 업체를 바꾸려 하면 차단
- **집행 가드** — 이미 집행된 예산을 줄이거나 되돌리려 하면 차단
- **승인 게이트** — 메일 발송·사례 적립 직전, 사람 승인(`ctx.ui` 확인)이 없으면 차단

즉 단순히 도구를 추가한 게 아니라, **AI가 값을 지어내거나 함부로 발송·변경하는 것을 코드 단에서 막는** Extension입니다.
또한 `estimate_budget`·`build_checklist`는 비율·일정을 코드에 박지 않고 **데이터 파일**(`ratios.json`·`checklist.json`)에서 읽어, Skill의 정책과 한 소스로 연결됩니다.
...
조건을 입력하면 에이전트가 **실시간으로 조사·도구 실행을 진행**하며, 각 단계에 어떤 Pi 요소를 썼는지 마커(MCP·Extension·Skill)가 달립니다. 상단에 *"MCP ×3 · Extension ×5 · Skill ×1 · 서브에이전트 ×5"* 요약이 표시되어, 화면에서 5요소가 실제로 동작함을 보여 줍니다.

![에이전트 작업 로그](docs/screenshots/agent-timeline.png)

### 홈 대시보드

진행 중인 행사 타일·알림·일정 캘린더·할 일 체크리스트를 한눈에 봅니다.

![홈 대시보드](docs/screenshots/home.png)

### 작업 공간 (상태 모델)

행사 한 건의 예산·장소(지도)·날씨·일정을 관리합니다. 예산은 *계획 → 확정🔒 → 집행* 단계로 추적됩니다.

![작업 공간](docs/screenshots/workspace.png)

### 새 행사 만들기

장소·인원·예산·날짜 조건을 입력해 기획을 시작합니다.
...
> 인용 드로어는 제안서의 `## 출처` 섹션을 파싱해 `[n]` 클릭 시 출처 텍스트와 사례·원문 링크를 보여줍니다. 일부 상세 화면은 추후 보강 예정입니다.
...
아래가 모두 준비되어 있어야 합니다(설치 절차는 [6. 설치 방법](#6-설치-방법)).
...
- [ ] **`.env`에 `OPENROUTER_API_KEY`** — ⚠️ **필수.** 없으면 에이전트(LLM)가 전혀 동작하지 않습니다(빈 화면만 뜸).
- [ ] (선택) **외부 도구용 Google 키** — Gmail·Calendar·Maps. **없어도 앱은 실행**되며 외부 도구만 비활성화됩니다.

> 즉 설치를 마친 뒤엔 **OpenRouter 키 하나만으로 핵심 기능(에이전트 대화·기획·제안서)이 동작**합니다. 메일·캘린더·지도 연동은 선택 사항입니다.

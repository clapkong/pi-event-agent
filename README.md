...
├─ frontend/                       # React + Vite 프런트 (홈·기획폼·타임라인, F0–F3 구현·mock 구동)
├─ backend/                        # 얇은 Node + WebSocket + Pi 세션 + 모델 레지스트리 (예정)
...
프런트엔드를 mock으로 단독 구동 중 (외부 의존 0). 백엔드가 같은 계약을 구현하면 mock만 교체된다.

- **UI 구현(mock — 실제 기능 아님)**: 사이드바 셸 · 홈 대시보드 · 기획 폼 · 동작 타임라인
- **예정**: 작업공간/상태 모델 · 결과 문서 · 백엔드(Pi 세션·Extension·Skill·MCP)

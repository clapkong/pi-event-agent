...
Pi 기반 **행사 기획 AI 에이전트 웹 서비스**(오픈소스 SW 수업 기말). **구현 진행 중** — `frontend/` F0–F5 완성, **백엔드 B 구현·브라우저 실증**(`backend/`: `pi --mode rpc`↔WS 브리지). 에이전트 대화는 실연결, 데이터 화면(home·board·doc·cases)은 아직 mock(REST 미구현). 정본·진행은 `plans/`(로컬: `PLAN.md`·`TASKS.md`), 남은 일은 `TODO.md`.

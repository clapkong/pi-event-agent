...
Pi 기반 **행사 기획 AI 에이전트 웹 서비스**(오픈소스 SW 수업 기말). **구현 진행 중** — `frontend/`는 S3 타임라인까지 완료(F0·F1·F2), `backend/`는 예정. 레포 골격은 `.pi`·`extensions`·`skills`·`.mcp.json`. 진행 현황은 `plans/TASKS.md`.

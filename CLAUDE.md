...
Pi 기반 **행사 기획 AI 에이전트 웹 서비스**(오픈소스 SW 수업 기말). **구현 진행 중** — `frontend/`는 F0–F4(홈·기획폼·타임라인·작업공간, mock), `backend/`는 V0·B1·B2 완료(**실제 Pi 연동 = M1**). 남은 것: 프런트 F5, 백엔드 B3(Extension·Skill·MCP·pgvector)·B4. 단계 정의는 `docs/BUILD.md`, 진행 현황은 `docs/TASKS.md`, **다음 세션 시작점은 `plans/NEXT_SESSION.txt`**.
...
정본·문서 맵은 **`plans/PLAN.md` §0**(로컬). 거기서 시작하면 나머지 문서로 이어진다. 코딩 전엔 `docs/TASKS.md`(단계 정의 `docs/BUILD.md`), 작업 후엔 `docs/ai-usage-log.md`에 한 줄. 과제 명세 최종 권위는 `plans/DETAILS.md`.

...
Pi 기반 **행사 기획 AI 에이전트 웹 서비스**(오픈소스 SW 수업 기말). **구현 진행 중** — `frontend/`는 F0–F5(홈·기획폼·타임라인·작업공간·문서, 전부 mock) 완성. **백엔드는 Pi부터 새로 작성 예정** — 재시작 핸드오프는 `plans/BACKEND_RESTART.txt`(로컬). 진행 현황은 `docs/TASKS.md`(공개), 정본은 `plans/PLAN.md`(로컬).

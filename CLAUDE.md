...
> 실데이터: ✅ 에이전트(`realClient`, WS) + ✅ 데이터 화면(REST `/api/*` ← `data/workspace/<id>/state.json`·`data/cases/*.md`) 모두 백엔드 연결 완료. mock seed 제거됨. 프런트 백엔드 접점 = `frontend/src/agent/contract.ts`(에이전트 WS 계약) + `/api/*`(데이터 REST).

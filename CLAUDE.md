...
Pi 기반 **행사 기획 AI 에이전트 웹 서비스**(오픈소스 SW 수업 기말). **구현 진행 중** — `frontend/`는 F0–F3(홈·기획폼·타임라인, mock), `backend/`는 V0·B1·B2 완료(**실제 Pi 연동 = M1**). 남은 것: 프런트 F4·F5, 백엔드 B3(Extension·Skill·MCP·pgvector)·B4. 진행 현황은 `plans/TASKS.md`, **다음 세션 시작점은 `plans/NEXT_SESSION.txt`**.
